...
export function createCopyMode(input: {
  ...
  function sourceLine(node: any, src: number): string | undefined {
    const content =
      typeof node.content === "string" ? node.content : typeof node._content === "string" ? node._content : undefined
  ...
  function stripRenderedListPrefix(text: string, prefix: string): string {
    const task = prefix.match(/\[[ xX]\]\s+$/)?.[0]
    if (task && text.toLowerCase().startsWith(task.toLowerCase())) return text.slice(task.length)
    if (task) return text.replace(/^\[[^\]]+\]\s+/, "")
    return text.replace(/^(?:[-+*•◦‣]\s+|\d{1,9}[.)]\s+)/, "")
  }

  function prefixedText(raw: string | undefined, visiblePrefix: string, text: string): string {
    const prefix = markdownListPrefix(raw)
    if (prefix) return text.startsWith(prefix) ? text : prefix + stripRenderedListPrefix(text, prefix)
    return visiblePrefix + text
  }
  ...
  function copyLine(row: CopyRow, child: any): { text: string; col: number } {
    ...
    if (info?.lineSources && local < info.lineSources.length) {
      ...
      if (!wrapped) return { text: prefixedText(sourceLine(match.node, src), prefix, source), col: match.gutter }
      ...
      return {
        text: prefixedText(offset === 0 ? sourceLine(match.node, src) : undefined, prefix, sliceCols(source, offset, width)),
        col: match.gutter,
      }
    }
    if (local >= lines.length) return { text: "", col: match.gutter }
    return { text: prefixedText(sourceLine(match.node, local), prefix, lines[local] ?? ""), col: match.gutter }
  }
